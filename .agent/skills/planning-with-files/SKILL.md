...
2.  **The 2-Action Rule**: Record findings in `findings.md` after every _two_ major research operations (web searches, large file reads).
...
4.  **Context Rebuild**: If a session is interrupted, re-read all three files _before_ continuing to restore full mental context.
...
_Note: This skill is inspired by the OthmanAdi/planning-with-files workflow._
