...
- 最新变更: 主数据身份治理 WP2 已为合格率建立 generation/CAS 原子发布、领域窄投影、固定事实快照影子对账和默认关闭的报表开关；读路径以数据库端新鲜度保护拒绝陈旧投影，用户可见口径仍为 legacy，生产环境未访问、未修改。
- 测试状态: WP2 定向 `13/13` 测试、全仓 Vitest `259/259` 文件、`2371/2371` 用例及全部提交门禁均已通过。
