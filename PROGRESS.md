...
- 报检任务显式责任契约已完成：三类责任均持久化 `responsibilityType + responsibleDepartmentId + responsibleDepartment`，外部复用 `supplierId`。PROCESS internal 必须提交 TEAM 与部门；SUPPLIER/OUTSOURCING_UNIT 直接提交 supplier 与 policy department、没有 TEAM/link 依赖。Web/WeApp、public/auth options、创建/查询/统计/检验记录均消费同一 canonical 事实。关闭报检锁定完整 context；仅 raw triad 全空的 legacy 请求可在事务内 CAS 裁决，partial triad 拒绝。历史回填使用 ID keyset、CAS 和 OPEN 审计；`MISSING_CANONICAL_RESPONSIBILITY_EVIDENCE` 是已完整扫描并审计的 nullable legacy 兼容结果，不是 skip，失效证据、冲突、CAS 并发或截断才阻断。实现提交：`40070cf`、`d84297c`、`1704d7d`、`8e33910`。独立验证：backend targeted `10/10` 文件、`102/102` 用例（含 backfill `2/2`、`15/15`）；Web changed tests `4/4`、`35/35`，WeApp `3/3`、`14/14`，修正后独立复跑 Web `1/1`、`16/16` 与 WeApp `1/1`、`7/7`（agent 汇总 `55/55`）。最终门禁已通过：backend 全量 `275/275` 文件、`2526/2526` 用例，Web `57/57`、`287/287`，WeApp `8/8`、`30/30`；shared build、`pnpm lint`、`pnpm run check:type`、`pnpm run check:qms-arch`、`pnpm run check:qms-arch:all`、backend/web typecheck 和 diff 均 PASS。浏览器/小程序真实点击仍未验证（访问 `localhost:5320` 被客户端阻止，Chrome 不可用）；真实运行库 dry-run/apply 未执行。
