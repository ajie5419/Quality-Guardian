...
- 最新变更: 历史统计统一采用 canonical ID 优先、原始名称快照兜底；售后、不合格项、质量概览、项目/供应商排行、日报周报和车辆故障率均保留旧数据。新建和分类变更继续强制 ID，治理写回增加事务内校验与并发条件更新。生产环境未访问、未修改。
- 测试状态: 全仓单元测试 `334/334` files and `2859/2859` tests passed；Backend full suite `245/245` files and `2284/2284` tests passed。
