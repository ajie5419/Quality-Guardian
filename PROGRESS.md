...
- 最新变更: 完成报检不合格项事业部根因修复；部门 ID/name 双写、事务内检验关联、历史回填与部署自动执行已贯通
- 测试状态: 本次后端 219/219 个文件、2040/2040 个用例通过；前端定向 2/2 个文件、14/14 个用例通过
- Lint: 通过（0 error，0 warning）
- Typecheck: 0 error（3/3 workspace tasks；weapp 自身脚本为项目既有 skip）
- 模块 TS 文件数: 513（含测试）
- 当前版本: `0.17.5`（本地修复已完成，待后续发布）
...
├── modules/      # 业务逻辑（25 个模块，512 个 TS 文件）
...
- [ ] 持续补强端到端业务流程验证
- [ ] 发布后核对事业部回填汇总及新增 OPEN 审计，人工处置重名、冲突或无证据的历史记录
