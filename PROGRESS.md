...
- 最新变更: 微信小程序不合格品项登记模块已实施，复用电脑版业务流程与后端数据
- 测试状态: 后端 198 文件 / 1871 测试全部通过；不合格品项定向 117/117 通过
...
├── modules/      # 业务逻辑（25 个模块，491 个 TS 文件）
...
- [ ] 在微信开发者工具和真机完成不合格品项全流程验收（权限入口、新增、照片上传、编辑、分页、草稿、账号切换）
- [ ] 将既有不合格品项统计趋势 SQL 改造为完整支持行级 data scope；当前已有 List RBAC，但趋势聚合仍是历史全量口径
- [ ] 持续补强端到端业务流程验证
