...
- 最新变更: 新增独立物料主数据与“物料新增申请 → 后台审核 → 生成或关联规范 ID”闭环；公开进货报检支持申请未建档物料，审核前不暴露卡滞状态且不可派单，审核通过后回填规范 ID 并立即进入正常派单链路。生产环境未访问、未修改。
- 测试状态: 全仓单元测试 `340/340` files and `2893/2893` tests passed；Backend full suite `246/246` files and `2304/2304` tests passed。
