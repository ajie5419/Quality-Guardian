...
- 质量损失索引发布可靠性根因已闭环：`quality_loss_index` 不再依赖每次发版触发异步全量 backfill。四类来源在各自数据库事务内 enqueue 持久化索引任务；独立 worker 以 5 秒 lease 消费、重试并记录错误，历史任务由支持 dry-run/apply 的 enqueue 工具和独立 drain 工具处理。历史 enqueue、drain、投影重建及其他 remediation 均未接入同步 release maintenance。实现提交：`d567fb65`、`4715c88c`；发布维护/发布器保障提交：`4fce2d15`、`8da5c1a1`、`f54859d7`、`6cf0f3e1`、`22cc54e4`、`8f08c0de`。完整后端 Vitest `285/285` 文件、`2596/2596` 用例，`pnpm lint`、`pnpm run check:type`、`pnpm run check:qms-arch`、Prisma validate、migration checks、发布 shell 行为测试和 `rtk git diff --check` 均通过；生产 migration、release maintenance、历史 enqueue/apply 与 drain 均未执行。
