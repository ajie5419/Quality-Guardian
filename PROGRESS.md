...
- 发布可用性修复已完成：Prisma migration 与 release maintenance 通过新镜像 one-off 容器执行期间，旧 backend 保持在线；仅当前置阶段成功后才停止旧 backend 并启动新服务。回滚继续恢复旧 compose 配置，但 migration、maintenance 失败或超时时不会重启未停止的旧 backend；只有已进入切换阶段的失败路径才拉起旧 backend。shell 行为测试、语法检查、`pnpm lint`、`pnpm run check:type`、`pnpm run check:qms-arch` 和 `pnpm run check:prisma-migration` 均通过；生产发布尚未执行。
