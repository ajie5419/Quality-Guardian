...
- 历史报检外协责任已按入口契约统一修复：`INCOMING` 与 `PROCESS` 的 Web、H5 和小程序入口/关单都隐藏并省略 `responsibleDepartmentId`，仅选择外协单位；后端从同一系统设置解析 canonical “生产 OBU”部门并写入请求、检验记录和 FAIL 不合格项。历史外协任务只有在已持久化类型、部门 ID 和供应商 ID 均完整时才锁定；仅有供应商但缺部门时会进入裁决流程，PASS/FAIL 顶层责任与 FAIL `linkedIssue` 都不提交部门。显式伪造外协部门、配置缺失/失效/歧义、既有责任冲突继续 fail-closed。实现提交：`e32d466`、`04141cc`、`f2f0e7f`。最终验证：后端全量 Vitest `288/288` 文件、`2621/2621` 用例，Web happy-dom Vitest `65/65` 文件、`340/340` 用例，WeApp Vitest `10/10` 文件、`52/52` 用例；`pnpm lint`、`pnpm run check:type`、`pnpm run check:qms-arch`、`pnpm run check:qms-arch:all`、`pnpm run check:prisma-migration` 和 `rtk git diff --check` 均 PASS。未运行前端 dev/build/start、真实登录页面或生产数据关单。
...
- PROCESS 内部报检的业务“班组”已收敛为 canonical `responsibleDepartment`：Web/WeApp 入口不再展示或提交执行 TEAM，V2 后端拒绝 `team/teamId`。PROCESS 仅允许内部部门或外协单位；外协入口隐藏责任部门，创建事务从系统设置解析活跃 canonical 部门。请求 API 和检验记录 API 共用同一后端展示规则：PROCESS 内部显示责任部门，PROCESS 外协显示 canonical `supplierName`，外部/历史行仍返回真实 TEAM；绝不伪造 TEAM ID。关单事务以报检任务完整 R（责任类型、部门 ID/名称、供应商 ID/名称）为唯一事实，逐条投影新建多工单记录、空事实的显式关联记录和 FAIL 不合格项；历史 partial 可由显式裁决补齐，冲突继续 fail-closed。历史 inspection 自身缺失时仅在关联报检任务唯一同部门时兼容展示，冲突保持空。未运行 dev/build/start，未提交或发布。
...
- 报检入口责任部门默认值已在 Web 与 WeApp 统一：`OUTSOURCING_UNIT` 不再由客户端按“生产 OBU”名称预选部门，而是始终隐藏部门并由后端系统设置解析；`SUPPLIER` 仍可对“采购部”做唯一精确匹配。共享纯函数保留供应商同类型的手动部门选择；空字段在选项未加载、零匹配或重名时保持未选并由提交校验 fail-closed，不硬编码部门 ID。
- CI Unit Tests 工作目录依赖已修复：release maintenance manifest 测试改由 `import.meta.url` 解析同目录 runner，不再依赖仓库根或 `apps/backend` 的 `cwd`。根目录与 `apps/backend` 的定向 Vitest 均通过（各 `1/1` 文件、`4/4` 用例）；supplier identity 定向 Vitest `3/3` 文件、`50/50` 用例，后端全量 Vitest `285/285` 文件、`2596/2596` 用例，根目录全量 unit tests `393/393` 文件、`3268/3268` 用例均通过；`pnpm lint`、`pnpm run check:type`、`pnpm run check:qms-arch`、`pnpm run check:qms-arch:all`、`pnpm run check:prisma-migration`、`rtk git diff --check` 均通过，独立提交待完成。
- 发布可用性修复已完成：Prisma migration 与 release maintenance 通过新镜像 one-off 容器执行期间，旧 backend 保持在线；前置阶段成功后不再显式停止 backend，而由一次 Compose 启动命令按新镜像重建服务。切换命令前记录 `backend_switch_started`，因此切换或健康检查失败时回滚会恢复旧 compose 并再次启动 backend；migration、maintenance 失败或超时时不会对未切换的旧 backend 执行启动操作。shell 行为测试、语法检查、`pnpm lint`、`pnpm run check:type`、`pnpm run check:qms-arch` 和 `pnpm run check:prisma-migration` 均通过；单实例 Compose 重建仍有短暂连接中断，生产发布尚未执行。
- 质量损失索引发布可靠性根因已闭环：`quality_loss_index` 不再依赖每次发版触发异步全量 backfill。四类来源在各自数据库事务内 enqueue 持久化索引任务；独立 worker 每 5 秒轮询，以 5 分钟 lease 消费、重试并记录错误，历史任务由支持 dry-run/apply 的 enqueue 工具和独立 drain 工具处理。历史 enqueue、drain、投影重建及其他 remediation 均未接入同步 release maintenance。实现提交：`d567fb65`、`4715c88c`；发布维护/发布器保障提交：`4fce2d15`、`8da5c1a1`、`f54859d7`、`6cf0f3e1`、`22cc54e4`、`8f08c0de`；文档记录：`55062b80`。完整后端 Vitest `285/285` 文件、`2596/2596` 用例，`pnpm lint`、`pnpm run check:type`、`pnpm run check:qms-arch`、Prisma validate、migration checks、发布 shell 行为测试和 `rtk git diff --check` 均通过；生产 migration、release maintenance、历史 enqueue/apply 与 drain 均未执行。
- 发布维护已完成根因修复：旧的永久 shell 清单已替换为版本化 manifest 与持久化 ledger。常规发布只领取 manifest 声明且未完成的启动前置幂等任务；已完成任务跳过，失败或过期租约允许后续发布重试，`taskKey + revision` 的 SHA-256 checksum 漂移 fail-closed。sidecar、历史 remediation、投影重建和窗口/评分对账已明确移出同步发布。远端发布器使用固定 one-off 容器、preflight、有界阶段和失败回滚；生产重试前必须人工确认并清理已确认的旧随机名残留。实现提交：`4fce2d15`、`8da5c1a1`。本轮仅完成本地自动化验证，尚未在生产执行 migration、maintenance 或失败恢复演练。
...
- 报检入口责任契约已与不合格问题报告对齐并完成浏览器验收：进货入口可选三类责任类型，过程入口仅允许内部部门和外协单位；内部部门与供应商责任使用 canonical department 下拉，外协责任隐藏部门并由服务端设置解析。外部类型提交时服务端校验 canonical 供应商类别与责任类型一致。原实现提交：`6744d71`（后端/共享契约）、`d775ffc`（Web 入口）、`ed8508f`（WeApp 入口）；外协隐藏部门的两类别统一由本轮修复覆盖。
