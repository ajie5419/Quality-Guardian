...
- 最新变更: TEAM 合并并发一致性、报检工序类别、BOM 关系更新、质量损失部门写入和图表消费端已完成 canonical ID 全链路治理。生产环境仍保持 `qgs-v0.19.1`，未连接或修改生产数据。
- 测试状态: 后端全量 `232/232` 个测试文件、`2200/2200` 个用例通过；本轮定向测试全部通过。
