...
export const defaultResponseInterceptor = ({
  ...
  dataField: ((response: ResponseData) => unknown) | string;
      ...
      if (status >= 200 && status < 400) {
        if (config.responseReturn === 'body') {
          ...
          isFunction(successCode)
            ? successCode(typedResponseData[codeField])
            : typedResponseData[codeField] === successCode
        ) {
          return isFunction(dataField)
            ? dataField(typedResponseData)
            : typedResponseData[dataField];
        }
      }
