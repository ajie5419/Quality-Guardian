...
const DFMEA_ID_ALPHABET = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ';
const DFMEA_ID_SUFFIX_SIZE = 6;

function createDfmeaIdSuffix(size = DFMEA_ID_SUFFIX_SIZE) {
  let output = '';
  for (let index = 0; index < size; index += 1) {
    const randomIndex = Math.floor(Math.random() * DFMEA_ID_ALPHABET.length);
    output += DFMEA_ID_ALPHABET[randomIndex];
  }
  return output;
}
...
export function createDfmeaProjectId(): string {
  return `DFMEA-${createDfmeaIdSuffix()}`;
}
