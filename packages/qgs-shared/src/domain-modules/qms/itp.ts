...
const ITP_PLAN_STATUS_VALUES = new Set<ItpPlanStatus>([
  'ACTIVE',
  'APPROVED',
  'ARCHIVED',
  'DRAFT',
...
const ITP_PROJECT_ID_ALPHABET = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ';
const ITP_PROJECT_ID_SUFFIX_SIZE = 6;

function createItpProjectIdSuffix(size = ITP_PROJECT_ID_SUFFIX_SIZE) {
  let output = '';
  for (let index = 0; index < size; index += 1) {
    const randomIndex = Math.floor(
      Math.random() * ITP_PROJECT_ID_ALPHABET.length,
    );
    output += ITP_PROJECT_ID_ALPHABET[randomIndex];
  }
  return output;
}
...
export function createItpProjectId(): string {
  return `ITP-PROJ-${createItpProjectIdSuffix()}`;
}

export function normalizeItpPlanStatus(value: unknown): ItpPlanStatus {
  const normalized = String(value ?? '')
    .trim()
    .toUpperCase();
  if (ITP_PLAN_STATUS_VALUES.has(normalized as ItpPlanStatus)) {
    return normalized as ItpPlanStatus;
  }
  return 'DRAFT';
}

export function toItpPlanStatusText(
  value: ItpPlanStatus | null | string,
): string {
  if (!value) {
    return 'draft';
  }
