...
const BOM_ID_ALPHABET = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ';
const BOM_ID_SUFFIX_SIZE = 6;

function createBomIdSuffix(size = BOM_ID_SUFFIX_SIZE) {
  let output = '';
  for (let index = 0; index < size; index += 1) {
    const randomIndex = Math.floor(Math.random() * BOM_ID_ALPHABET.length);
    output += BOM_ID_ALPHABET[randomIndex];
  }
  return output;
}

export function createBomItemId(): string {
  return `BOM-${createBomIdSuffix()}`;
}

export function createBomProjectId(): string {
  return `BOM-PROJ-${createBomIdSuffix()}`;
}
...
function normalizeBomProcessList(value: unknown): string[] {
  ...
  return [
    ...new Set(values.map((item) => normalizeBomText(item)).filter((v): v is string => Boolean(v))),
  ];
}
