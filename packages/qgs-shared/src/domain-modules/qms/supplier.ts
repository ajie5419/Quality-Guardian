...
const SUPPLIER_ID_ALPHABET = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ';
const SUPPLIER_ID_SUFFIX_SIZE = 6;

function createSupplierIdSuffix(size = SUPPLIER_ID_SUFFIX_SIZE) {
  let output = '';
  for (let index = 0; index < size; index += 1) {
    const randomIndex = Math.floor(Math.random() * SUPPLIER_ID_ALPHABET.length);
    output += SUPPLIER_ID_ALPHABET[randomIndex];
  }
  return output;
}
...
export function createSupplierId(now = new Date()): string {
  return `SUP-${now.getFullYear()}-${createSupplierIdSuffix()}`;
}
