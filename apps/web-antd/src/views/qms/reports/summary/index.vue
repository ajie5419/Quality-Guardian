  ...
  if (reportType.value === 'weekly') {
    ...
    if (element) {
      try {
        ...
        link.href = url;
        link.download = `Weekly_Report_${weeklyRange.value.start}.png`;
        link.click();
                ...
                >
                  ...
                  <div class="flex-1">
                    <div class="mb-2 flex items-start justify-between">
                      ...
                      <Tag
                        :color="event.status === 'CLOSED' ? 'blue' : 'orange'"
                      >
                        {{ event.status }}
                      </Tag>
                    </div>
                    ...
                    <ul class="list-disc space-y-3 pl-4 text-sm">
                      <li v-if="(reportData.metrics?.[0]?.trend ?? 0) < 0">
                        ...
                        <b class="text-yellow-300"
                          >{{
                            Math.abs(reportData.metrics?.[0]?.trend ?? 0)
                          }}%</b
                        >)，{{
