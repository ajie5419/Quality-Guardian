...
import { getQualityLossList, deleteQualityLoss } from '#/api/qms/quality-loss';
...
import { STATUS_OPTIONS, SOURCE_STYLE_MAP } from './constants';
...
const gridOptions: VxeGridProps = {
  ...
  columns: [
    ...
    { field: 'workOrderNumber', title: t('qms.workOrder.workOrderNumber'), width: 120 },
    { field: 'projectName', title: t('qms.workOrder.projectName'), minWidth: 150 },
    { field: 'partName', title: t('qms.inspection.issues.partName'), minWidth: 150 },
    ...
    { title: t('common.action'), width: 130, fixed: 'right', slots: { default: 'action' } },
  ],
  proxyConfig: {
    ajax: {
      query: async ({ page }) => {
        ...
        <Grid>
          ...
          <template #lossSource="{ row }">
            <Tag :color="SOURCE_STYLE_MAP[row.lossSource as LossSource]?.color">
              {{ t(SOURCE_STYLE_MAP[row.lossSource as LossSource]?.labelKey || row.lossSource) }}
            </Tag>
          </template>
