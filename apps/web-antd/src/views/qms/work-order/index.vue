...
import { getGridColumns, type WorkOrderFormValues } from './data';
...
const { handleImport, loading: importLoading } = useWorkOrderImport(() => {
  gridApi.value?.reload();
});

// 6. Grid 表格配置
const gridOptions = computed<VxeGridProps<QmsWorkOrderApi.WorkOrderItem>>(() => ({
  columns: [
  ...
  proxyConfig: {
    ajax: {
      ...
      queryAll: async ({ formValues }: { formValues: WorkOrderFormValues }) => {
        try {
          const response = await getWorkOrderList({
            year: currentYear.value,
            ...formValues,
...
function handleBatchDelete() {
  ...
  Modal.confirm({
    ...
    content: t('qms.common.confirmBatchDeleteContent', { count: checkedRows.value.length }),
    onOk: async () => {
      try {
        // 类型安全：后端使用 workOrderNumber 作为主键
        const ids = checkedRows.value.map((r: QmsWorkOrderApi.WorkOrderItem) => r.workOrderNumber);
      ...
      >
        <Grid>
          ...
          <template #toolbar-actions>
            <Space>
              <Button v-if="canCreate" shape="round" type="primary" @click="handleAdd">
                <template #icon>
              ...
              <Button @click="showDashboard = !showDashboard">
                <template #icon>
                  <IconifyIcon :icon="showDashboard ? 'lucide:layout-panel-top' : 'lucide:layout-panel-off'" />
                </template>
                {{ showDashboard ? t('qms.workOrder.hideChart') : t('qms.workOrder.showChart') }}
              </Button>
