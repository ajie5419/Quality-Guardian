...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    await recordBusinessAuditLog(event, { userId: userinfo.id, action: 'CREATE', targetType: 'supplier', targetId: 'batch-import', detailsTemplate: '导入供应商/外协单位: {{successCount}}/{{totalCount}} 条', detailsVariables: { successCount: result.successCount, totalCount: result.totalCount } });
