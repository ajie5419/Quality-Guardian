...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    await recordBusinessAuditLog(event, { userId: userinfo.id, action: 'UPDATE', targetType: 'supplier', targetId: String(id), detailsTemplate: '修改供应商/外协单位: {{name}}', detailsVariables: { name: updated.name } });
