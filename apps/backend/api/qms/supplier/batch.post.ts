...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    await recordBusinessAuditLog(event, { userId: userinfo.id, action: 'CREATE', targetType: 'supplier', targetId: 'batch-upsert', detailsTemplate: '批量导入供应商/外协单位: 成功 {{success}} 条，跳过 {{skipped}} 条，失败 {{errors}} 条', detailsVariables: { errors: results.errors, skipped: results.skipped, success: results.success } });
