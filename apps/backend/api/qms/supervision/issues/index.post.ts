...
const createIssueBodySchema = z.object({ description: z.unknown().optional(), photos: z.array(z.any()).optional(), projectId: z.unknown().optional() }).passthrough();
...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    return internalServerErrorResponse(event, 'Failed to create supervision issue');
  }
});
