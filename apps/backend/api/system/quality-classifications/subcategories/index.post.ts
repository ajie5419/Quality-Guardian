  ...
  async (event, body) => {
    try {
      ...
      return useResponseSuccess(
        await QualityClassificationService.createSubcategory(body),
      );
    } catch (error: unknown) {
      logApiError(
        'quality-classification-subcategory-create',
        error,
        undefined,
        event,
      );
      ...
      if (isPrismaUniqueConflictError(error)) {
        return conflictResponse(
          event,
          'Subcategory code or name already exists',
        );
      }
      return internalServerErrorResponse(event, 'Failed to create subcategory');
    }
  },
);
