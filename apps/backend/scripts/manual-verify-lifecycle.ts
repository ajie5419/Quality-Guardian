...
async function demo(now: Date) {
  ...
  console.log(`  ① 已构造过期记录: ${sample.id}（retainUntil 设为 1 分钟前）`);
  ...
  console.log('  ④ 已还原测试数据（archivedAt/retainUntil 置空）');
  console.log(`  演示${ok ? '通过' : '失败'}${ok ? ' ✅' : ' ❌'}`);
}
...
main()
  .catch((error) => {
    logger.error({ err: error }, 'lifecycle verify failed');
    process.exitCode = 1;
  })
  .finally(async () => {
