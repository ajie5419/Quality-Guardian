...
interface TableCoverageReport {
  ...
  skippedNonFieldReads: number;
  table: string;
  totalReadHits: number;
}
...
function isCanonicalReadContextSnippet(snippet: string) {
  const normalized = snippet.replaceAll(/\s+/g, ' ');
  return (
    normalized.includes('resolveCanonical') ||
    normalized.includes('buildProcessNameWhere') ||
    normalized.includes('MasterDataGovernanceKernel.resolveCanonical') ||
    normalized.includes('governance-allow-direct-canonical-read')
  );
}

function hasCanonicalReadContext(lines: string[], start: number, end: number) {
  const contextStart = Math.max(0, start - 40);
  const contextEnd = Math.min(lines.length - 1, end + 40);
  for (let index = contextStart; index <= contextEnd; index += 1) {
    if (isCanonicalReadContextSnippet(lines[index])) {
      return true;
    }
  }
  return false;
...
function resolveReadCallEnd(lines: string[], start: number) {
  const hardLimit = Math.min(lines.length - 1, start + 240);
  ...
  for (let index = start; index <= hardLimit; index += 1) {
    ...
    for (const char of line) {
      if (char === '(') {
        seenOpenParen = true;
        parenDepth += 1;
      } else if (char === ')' && seenOpenParen) {
        parenDepth = Math.max(0, parenDepth - 1);
      }
    }
    if (seenOpenParen && parenDepth === 0 && index >= start) {
      return index;
    }
  }
  return hardLimit;
...
function collectTableReadHits(table: string, backendDir: string) {
  const pattern = `${quoteForRegex(table)}\\.${String.raw`(?:findMany|findFirst|findUnique|findUniqueOrThrow|aggregate|groupBy)\(`}`;
  ...
  if (result.status !== 0 && result.status !== 1) {
    throw new Error(
      `[audit-master-data-read-coverage] rg failed for table ${table}: ${String(
...
function buildTableFieldMap(fields: MasterDataGovernanceField[]) {
  const map = new Map<
    string,
    { canonicalFieldKeys: Set<string>; targetColumns: Set<string> }
  >();
  for (const field of fields) {
    for (const target of field.targets) {
      const entry = map.get(target.table) || {
        canonicalFieldKeys: new Set<string>(),
        targetColumns: new Set<string>(),
      };
      entry.canonicalFieldKeys.add(field.key);
...
function resolveScopeFields(args: Map<string, string>) {
  const canonicalFields = listMasterDataGovernanceFields().filter(
    (field) => field.readStrategy === 'canonical-first',
  );
  ...
  if (fieldsArg) {
    ...
    return canonicalFields.filter((field) => selected.has(field.key));
  }
  ...
  return canonicalFields.filter((field) =>
    selectedWaves.has(field.rolloutWave),
  );
}
...
async function main() {
  ...
  if (fields.length === 0) {
    throw new Error('NO_CANONICAL_FIELDS_SELECTED');
  }
  ...
  for (const [table, metadata] of tableFieldMap.entries()) {
    const hits = collectTableReadHits(table, backendDir);
    const missingHits: CoverageHit[] = [];
    let skippedNonFieldReads = 0;
    for (const hit of hits) {
      const filePath = path.resolve(backendDir, hit.file);
      const fileLines = fs.readFileSync(filePath, 'utf8').split('\n');
      const start = Math.max(0, hit.line - 1);
      const end = resolveReadCallEnd(fileLines, start);
      const hasTargetField = hasTargetFieldRead(fileLines, start, end, [
        ...metadata.targetColumns,
      ]);
      if (!hasTargetField) {
        skippedNonFieldReads += 1;
        continue;
      }
      if (hasCanonicalReadContext(fileLines, start, end)) {
        continue;
      }
    ...
    reports.push({
      table,
      canonicalFieldKeys: [...metadata.canonicalFieldKeys].sort(),
      totalReadHits: hits.length,
      skippedNonFieldReads,
      missingHits,
    });
  }
  ...
  const summary = {
    ...
    scope: {
      ...
      canonicalFieldKeys: fields.map((field) => field.key),
    },
    totals: {
      tables: reports.length,
      totalReadHits: reports.reduce((sum, item) => sum + item.totalReadHits, 0),
      totalSkippedNonFieldReads: reports.reduce(
        (sum, item) => sum + item.skippedNonFieldReads,
        0,
      ),
  ...
  console.warn('[audit-master-data-read-coverage] result');
  ...
  const outPath = path.resolve(
    outDir,
    `read-coverage-${new Date()
      .toISOString()
      .replaceAll(':', '-')
      .replaceAll('.', '-')}.json`,
  );
  fs.writeFileSync(outPath, `${JSON.stringify(summary, null, 2)}\n`, 'utf8');
  console.warn('[audit-master-data-read-coverage] report');
...
main().catch((error: unknown) => {
  console.error('[audit-master-data-read-coverage] failed', error);
  process.exitCode = 1;
});
