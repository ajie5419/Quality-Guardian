...
export const ReportService = {
  async getWeeklyReport(
    ...
    try {
      ...
      processDeptNode(deptTree as any[]);
      const getDeptName = (id: null | string) =>
        id ? deptMap.get(id) || id : '-';

      // Transform Data
      const trackingIssues = trackingIssuesRaw.map((item) => ({
        id: item.id,
        type: '质量问题', // Default type
        ...
        completionTime: item.updatedAt.toISOString().split('T')[0],
        respDept: getDeptName(item.respDept),
        remarks: '',
      }));

      const internalIssues: IssueItem[] = internalIssuesRaw.map((item) => {
      ...
      const externalIssues: IssueItem[] = externalIssuesRaw.map((item) => {
        // Map severity to Chinese
        let level = '严重'; // Default for external
        const s = (item.severity || '').toLowerCase();
        if (
          s &&
          (s.includes('low') ||
        ...
        // Construct cause from failureCause or defectType/Subtype
      ...
      return {
        ...
        author: {
          name: 'System', // To be replaced by user context
          dept: 'Quality Dept',
          role: 'Manager',
          leader: 'Liu Pan', // Hardcoded from template
        },
        trackingIssues,
        internalIssues,
        externalIssues,
