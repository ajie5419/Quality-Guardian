    ...
    describe('getStats', () => {
        it('should aggregate statistics correctly', async () => {
            ...
            (prisma.work_orders.findMany as any).mockResolvedValue([
                { workOrderNumber: 'WO-1', projectName: 'P1', status: 'In Progress', customerName: 'C1' },
            ]);

            const stats = await DashboardService.getStats();
            const ql = stats.overview.qualityLoss as { total: number; weekly: number };
        ...
        it('should handle errors gracefully', async () => {
            (prisma.after_sales.aggregate as any).mockRejectedValue(new Error('DB Error'));
    ...
    describe('getMonthlyTrend', () => {
        it('should calculate pass rate correctly with BigInt values', async () => {
            (prisma.$queryRaw as any)
                ...
                .mockResolvedValueOnce([
                    { month: BigInt(1), defectQty: BigInt(5) },
                ]); // defects
