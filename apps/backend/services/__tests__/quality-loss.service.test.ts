...
describe('QualityLossService', () => {
    ...
    describe('getTrendData', () => {
        it('should handle trend data aggregation', async () => {
            (prisma.$queryRaw as any).mockResolvedValueOnce([{ p: 1, a: 100 }]) // manual
            ...
            const jan = result.trend.find(t => t.period === '1月' || t.period === 'Jan');
        ...
        it('should handle BigInt period and sum values', async () => {
            (prisma.$queryRaw as any).mockResolvedValueOnce([{ p: BigInt(5), a: BigInt(1000) }]) // manual
