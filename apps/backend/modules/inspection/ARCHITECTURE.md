...
- `SUPPLIER` 在 `INCOMING` 以及 `OUTSOURCING_UNIT` 在两类报检中都隐藏责任部门，只选择对应外部单位。历史任务只有在持久化责任类型、部门 ID 和供应商 ID 均完整时才锁定；缺少部门 ID 时必须显示裁决控件并提交无部门的顶层责任，由后端补齐匹配的配置部门。
...
`INCOMING` entry presents only `SUPPLIER` and `OUTSOURCING_UNIT`; `PROCESS` presents only `INTERNAL_DEPARTMENT` and `OUTSOURCING_UNIT`. Incoming entry never renders a department selector. It submits only the selected supplier or outsourcing-unit ID, and the server resolves the configured canonical `采购部` or `生产 OBU` department respectively. PROCESS internal responsibility still exposes a canonical department selector. No external responsibility resolves a department from client-visible names or options. V2 entry and submission never accept `team` or `teamId`; the existing nullable TEAM fields remain a legacy compatibility boundary only.
