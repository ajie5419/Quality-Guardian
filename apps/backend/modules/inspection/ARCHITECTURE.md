...
- `responsibilityType=SUPPLIER`：进货检验客户端只提交 `supplierId`；服务端从独立系统设置解析 canonical “采购部”，再按 canonical ID 重建部门和供应商名称。
...
`qms_inspection_requests` persists the nullable compatibility fields `responsibilityType`, `responsibleDepartmentId`, and `responsibleDepartment`; its existing `supplierId` is the external supplier fact. New requests must persist one complete canonical responsibility. For a PROCESS request with `INTERNAL_DEPARTMENT`, the canonical responsibility department is the business-facing team: entry does not collect `team` or `teamId`, and neither may be submitted through V2. `SUPPLIER` and `OUTSOURCING_UNIT` require only a client-selected external supplier ID; the department is resolved as the explicit client value when present, otherwise from the process default (`processes.responsibleDepartmentId`, maintained via `maintenance:process-responsible-departments`), and creation fails fast when neither exists. Neither external type carries a team. A new PROCESS external request therefore uses its persisted supplier ID directly and does not require a `TEAM -> supplier_identity_links` mapping. The former internal-option resolver incorrectly required a TEAM mapping before returning a department, hiding valid departments such as structure and machining BUs; responsibility department resolution is now independent of TEAM resolution.

The public and authenticated responsibility-options endpoints, Web entry form, and WeApp entry form all submit the same ID-only contract. `PROCESS` permits only `INTERNAL_DEPARTMENT` or `OUTSOURCING_UNIT`; `INCOMING` permits only `SUPPLIER` or `OUTSOURCING_UNIT`. Both incoming choices hide and omit `responsibleDepartmentId`. The external department is resolved at request creation from the client value if present, otherwise from the process default (`processes.responsibleDepartmentId`, set through the `maintenance:process-responsible-departments` release task); an unconfigured process fails creation with a clear operator hint. Query, inspection-record creation, and statistics consume the persisted external supplier fact rather than reconstructing it from a TEAM or a name. Close derives one complete request responsibility `R = responsibilityType + responsibleDepartmentId + responsibleDepartment + supplierId? + supplierName?` and, in the same transaction, projects the exact R snapshot to every newly generated inspection, an explicit pre-existing inspection with no responsibility fact, and the FAIL-linked NC. A close payload omitting the department inherits the request snapshot; a request snapshot without a department fails close. A compatible partial historical identity can be completed during PASS or FAIL only from a separately submitted responsibility decision. Stale names are refreshed, while type/department/supplier ID conflicts fail closed. One backend display function maps both request and inspection API `team`: incoming external requests and `PROCESS + OUTSOURCING_UNIT` return canonical `supplierName`, `PROCESS + INTERNAL_DEPARTMENT` returns `responsibleDepartment`, and legacy rows retain real `team`. This is a response-only projection: nullable `teamId` remains a distinct TEAM identity field, and a department ID or supplier ID is never written into it. Historical inspections whose own responsibility is missing may read a linked request department only when all non-deleted direct and join-table request links resolve to one identical internal department; multiple departments remain unresolved and display no arbitrary team. The standalone request-responsibility resolver prevents a dependency cycle between request context resolution and inspection-record creation.
...
- 独立不合格项表单保留三类责任类型；报检任务 FAIL 关单的嵌入式表单与报检入口类别契约一致：`INCOMING` 仅提供供应商和外协单位，`PROCESS` 仅提供内部部门和外协单位。
- `SUPPLIER` 在 `INCOMING` 以及 `OUTSOURCING_UNIT` 在两类报检中都隐藏责任部门，只选择对应外部单位。关闭时外部责任无需提交部门：后端优先使用提交值，否则继承报检单快照；报检单快照也缺部门时关闭失败并提示。历史任务只有在持久化责任类型、部门 ID 和供应商 ID 均完整时才锁定；缺少部门 ID 时必须显示裁决控件并提交无部门的顶层责任。
...
`INCOMING` entry presents only `SUPPLIER` and `OUTSOURCING_UNIT`; `PROCESS` presents only `INTERNAL_DEPARTMENT` and `OUTSOURCING_UNIT`. Incoming entry never renders a department selector. It submits only the selected supplier or outsourcing-unit ID, and the server resolves the department from the process default (`processes.responsibleDepartmentId`) at creation time. PROCESS internal responsibility still exposes a canonical department selector. No external responsibility resolves a department from client-visible names or options. V2 entry and submission never accept `team` or `teamId`; the existing nullable TEAM fields remain a legacy compatibility boundary only.
...
详情卡片也按同一类别边界展示身份：`INCOMING` 显示“供应商”和响应中的 `supplierName` 快照，缺失快照显示 `-`；任务列表的兼容 `team` 展示字段也从同一 `supplierName` 快照投影；`PROCESS + INTERNAL_DEPARTMENT` 显示责任部门解析出的业务“班组”，`PROCESS + OUTSOURCING_UNIT` 同样投影 `supplierName`，其他旧记录显示真实 `team`。该投影只存在于 API 响应，不得回写 `team` 或伪造 `teamId`。
