...
`qms_inspection_requests` persists the nullable compatibility fields `responsibilityType`, `responsibleDepartmentId`, and `responsibleDepartment`; its existing `supplierId` is the external supplier fact. New requests must persist one complete canonical responsibility. `PROCESS + INTERNAL_DEPARTMENT` requires an active internal `teamId` and its policy-approved department ID, with no supplier. `SUPPLIER` and `OUTSOURCING_UNIT` require a supplier ID and policy-approved department ID, with no team. A new PROCESS external request therefore uses its persisted supplier ID directly and does not require a `TEAM -> supplier_identity_links` mapping.

The public and authenticated responsibility-options endpoints, Web entry form, and WeApp entry form all submit the same ID-only three-state contract. Query, inspection-record creation, and statistics consume the persisted external supplier fact rather than reconstructing it from a TEAM or a name. The close flow locks the request's complete canonical context before it writes an issue. Only a raw responsibility triad that is entirely empty is legacy-compatible: the close transaction may establish the explicit fact with compare-and-set protection. Any partially populated triad is rejected, because it cannot be safely mixed with legacy inference.
