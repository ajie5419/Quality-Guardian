...
- 后端以显式类型和 canonical ID 为权威；历史报检任务的部分责任 identity 在 FAIL 关闭时可由表单提交、主数据验证后的 `responsibilityType + responsibleDepartmentId + supplierId?` 补全。已有非空 `responsibilityType`、`responsibleDepartmentId` 或 `supplierId` 必须与提交值一致，否则拒绝关闭；`responsibleDepartment` 和 `supplierName` 仅为名称快照，不作为 identity 冲突依据，并会按当前主数据刷新。
- PASS 关闭不得猜测责任：只有已有 identity 完整、且可由 canonical IDs 重新校验时才可关闭并刷新名称快照；identity 缺失仍拒绝。`PROCESS` 关闭禁止 `SUPPLIER`，只允许 `INTERNAL_DEPARTMENT` 或 `OUTSOURCING_UNIT`；外部供应商类别必须与责任类型匹配。
