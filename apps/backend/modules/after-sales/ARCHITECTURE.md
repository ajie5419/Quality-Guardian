...
- 新增、更新、删除和批量导入必须在源事务内为受影响的 `supplierIds` 追加评分刷新任务；名称集合不能触发下游刷新。
- 更新供应商关联时，源事务必须为旧 ID 和新 ID 同时追加持久化评分任务；删除时必须为被删除记录的旧 ID 追加任务，避免旧供应商快照残留。
...
- 售后评分读取和持久化任务派发已是 ID-only，供应商画像的售后查询按 `supplierBrandId` 精确过滤。
